...
## Migration steps
1. **Packages**:
   - Replace `grub` (and related dependencies like `os-prober`) with `limine` in `packages.x86_64`.
   - Keep `syslinux` for BIOS while Limine is validated.
   - Ensure `limine` binaries (`limine-bios.sys`, `limine-bios-cd.bin`, `limine-uefi-cd.bin`, `BOOTX64.EFI`) are available in the build environment.
...
5. **Build script updates**: In `build_iso.sh`, copy Limine assets from the host package into the temporary profile before invoking `mkarchiso`, then post-process the produced ISO with `limine bios-install <path-to-iso>` (covers both BIOS/UEFI images per upstream docs).
